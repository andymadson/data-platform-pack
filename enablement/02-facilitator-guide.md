...
- *"What stops an engineer from uninstalling the plugin?"* Nothing in the plugin itself, and that's deliberate. Managed settings and MDM are the enforcement layer; the plugin standardizes, and the audit trail shows where it was active.
- *"Can't the agent route around the regex with a wrapper script?"* Some variants will get through; the README says so out loud. The gate catches the highest-blast-radius patterns deterministically, fails to `ask` on its own errors, and sits inside defense in depth. Then take the wrapper pattern they named and ship it as a rule in beat 4.
